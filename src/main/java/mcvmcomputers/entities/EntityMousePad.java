...
public class EntityMousePad extends Entity {
	private static final TrackedData<Float> LOOK_AT_POS_X =
			DataTracker.registerData(EntityMousePad.class, TrackedDataHandlerRegistry.FLOAT);
	private static final TrackedData<Float> LOOK_AT_POS_Y =
			DataTracker.registerData(EntityMousePad.class, TrackedDataHandlerRegistry.FLOAT);
	private static final TrackedData<Float> LOOK_AT_POS_Z =
			DataTracker.registerData(EntityMousePad.class, TrackedDataHandlerRegistry.FLOAT);

	public EntityMousePad(EntityType<?> type, World world) {
		super(type, world);
	}

	public EntityMousePad(World world, double x, double y, double z) {
		this(EntityList.MOUSE_PAD, world);
		this.updatePosition(x, y, z);
	}

	public EntityMousePad(World world, Double x, Double y, Double z, Vec3d lookAt, String uuid) {
		this(EntityList.MOUSE_PAD, world);
		this.updatePosition(x, y, z);
	...
	public Vec3d getLookAtPos() {
		return new Vec3d(
				this.getDataTracker().get(LOOK_AT_POS_X),
