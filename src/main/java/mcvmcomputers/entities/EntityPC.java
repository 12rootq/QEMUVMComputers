...
public class EntityPC extends Entity{
	...
	protected void defineSynchedData(SynchedEntityData.Builder builder) {
		builder.define(HARD_DRIVE_FILE_NAME, "");
		builder.define(ISO_FILE_NAME, "");
		builder.define(OWNER_UUID, "");
		builder.define(LOOK_AT_POS_X, 0f);
		builder.define(LOOK_AT_POS_Y, 0f);
		builder.define(LOOK_AT_POS_Z, 0f);
		builder.define(GB_OF_RAM_IN_SLOT_0, 0);
		builder.define(GB_OF_RAM_IN_SLOT_1, 0);
		builder.define(CPU_DIVIDED_BY, 0);
		builder.define(GPU_IN_PCI_SLOT, false);
		builder.define(MOTHERBOARD_INSTALLED, false);
		builder.define(GLASS_SIDEPANEL, false);
		builder.define(SIXTY_FOUR_BIT, false);
	}
	@Override
	...
	public InteractionResult interact(Player player, InteractionHand hand) {
		if(!player.level().isClientSide()) {
			if(player.isCrouching() && player.getStringUUID().equals(this.getOwner())) {
