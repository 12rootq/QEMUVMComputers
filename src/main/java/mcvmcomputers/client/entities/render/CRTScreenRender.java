...
public class CRTScreenRender extends EntityRenderer<EntityCRTScreen>{
	...
	@Override
	public void render(EntityCRTScreen entity, float yaw, float tickDelta, PoseStack matrices,
		...
		Minecraft.getInstance().getItemRenderer().renderStatic(new ItemStack(ItemList.ITEM_CRTSCREEN), ItemDisplayContext.NONE, light, OverlayTexture.NO_OVERLAY, matrices, VertexConsumers, entity.level(), 0);
		if(ClientMod.vmScreenTextures.containsKey(UUID.fromString(entity.getOwnerUUID()))) {
			matrices.pushPose();
			matrices.scale(0.006f, 0.006f, 0.006f);
			matrices.mulPose(new Quaternionf().rotationXYZ((float)Math.toRadians(22.5f), 0f, 0f));
