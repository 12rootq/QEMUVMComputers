...
public class SetupPageIntroMessage extends SetupPage{
	...
	public void init() {
		if(!setupGui.loadedConfiguration) {
			...
			setupGui.addButton(ButtonWidget.builder(Text.literal(setupGui.translation("mcvmcomputers.setup.nextButton")), (bw) -> this.setupGui.nextPage()).dimensions(setupGui.width/2 - (buttonW/2), setupGui.height - 40, buttonW, 20).build());
		}else {
			...
			if(redoSetupW > useConfigW)
				w = redoSetupW;
			setupGui.addButton(ButtonWidget.builder(Text.literal(setupGui.translation("mcvmcomputers.setup.useConfig")), (bw) -> this.setupGui.lastPage()).dimensions(setupGui.width/2 - (w/2), setupGui.height / 2 - 25, w, 20).build());
			setupGui.addButton(ButtonWidget.builder(Text.literal(setupGui.translation("mcvmcomputers.setup.redoSetup")), (bw) -> this.delete()).dimensions(setupGui.width/2 - (w/2), setupGui.height / 2 + 5, w, 20).build());
		}
	}
	public void delete() {
