...
public class GuiPCEditing extends Screen {
	...
	public void renderBackgroundAndMobo(DrawContext context) {
		...
		ms.multiply(new Quaternionf().rotationAxis((float) Math.toRadians(6f * introScale), 0f, 0.70710678f, 0.70710678f)); // custom axis ~ (0, 0.1, 0.1)
	...
	public void render(DrawContext context, int mouseX, int mouseY, float delta) {
		...
		if(introScale > 0.99f) {
			if(openCase) {
				...
				if(SystemUtils.IS_OS_MAC) {
					context.drawText(this.textRenderer, lang.get("mcvmcomputers.pc_editing.put_panel_back_mac"), 4, 14, -1, false);
					if(minecraft.getWindow() != null && GLFW.glfwGetKey(minecraft.getWindow().getHandle(), GLFW.GLFW_KEY_RIGHT_ALT) == GLFW.GLFW_PRESS) {
						openCase = false;
						panelX = -panelX;
					}
				}else {
					context.drawText(this.textRenderer, lang.get("mcvmcomputers.pc_editing.put_panel_back"), 4, 14, -1, false);
				...
				if(pc_case.getMotherboardInstalled()) {
					...
					if(pc_case.get64Bit()) {
						context.drawText(this.textRenderer, lang.get("mcvmcomputers.64bit"), this.width/2 - 66, this.height/2 - 56, -1, false);
					}else {
						context.drawText(this.textRenderer, lang.get("mcvmcomputers.32bit"), this.width/2 - 66, this.height/2 - 56, -1, false);
					}
					ms.pop();
					...
					if(pc_case.getCpuDividedBy() == 0) {
						ms.push();
						ms.translate(0, 0, 200);
						context.drawText(this.textRenderer, lang.get("mcvmcomputers.pc_editing.add_cpu"), this.width/2 - 120, this.height/2 - 40, -1, false);
						ms.pop();

						int addCpuWidth = textRenderer.getWidth(lang.get("mcvmcomputers.pc_editing.add_cpu_btn").replace("%s", "6"));
						ButtonWidget div2 = ButtonWidget.builder(Text.literal(lang.get("mcvmcomputers.pc_editing.add_cpu_btn").replace("%s", "2")), (btn) -> this.addCPU(ItemList.ITEM_CPU2, 2))
								.dimensions(this.width/2 - (addCpuWidth+59), this.height / 2 - 31, addCpuWidth+4, 12).build();
						ButtonWidget div4 = ButtonWidget.builder(Text.literal(lang.get("mcvmcomputers.pc_editing.add_cpu_btn").replace("%s", "4")), (btn) -> this.addCPU(ItemList.ITEM_CPU4, 4))
								.dimensions(this.width/2 - (addCpuWidth+59), this.height / 2 - 18, addCpuWidth+4, 12).build();
						ButtonWidget div6 = ButtonWidget.builder(Text.literal(lang.get("mcvmcomputers.pc_editing.add_cpu_btn").replace("%s", "6")), (btn) -> this.addCPU(ItemList.ITEM_CPU6, 6))
					...
					if(!pc_case.getGpuInstalled()) {
						...
						ButtonWidget bw = ButtonWidget.builder(Text.literal(lang.get("mcvmcomputers.pc_editing.add_gpu")), (btn) -> this.addGPU())
					...
					if(pc_case.getHardDriveFileName().isEmpty()) {
						...
						ms.push();
						ms.translate(0, 0, 200);
						context.drawText(this.textRenderer, lang.get("mcvmcomputers.pc_editing.add_vhds"), this.width/2 + 20, this.height/2 + 30, -1, false);
						ms.pop();
						...
						if(count == 0) {
							ms.push();
							ms.translate(0, 0, 200);
							context.drawText(this.textRenderer, (char) (0xfeff00a7) + "7" + lang.get("mcvmcomputers.pc_editing.no_valid_vhd"), this.width/2 + 20, this.height/2 + 40, -1, false);
							ms.pop();
						}
					}else {
					...
					if(pc_case.getGigsOfRamInSlot0() == 0 || pc_case.getGigsOfRamInSlot1() == 0) {
						ms.push();
						ms.translate(0, 0, 200);
						context.drawText(this.textRenderer, lang.get("mcvmcomputers.pc_editing.add_ram"), this.width/2 + 50, this.height/2 - 60, -1, false);
						ms.pop();

						int addMBRamWidth = textRenderer.getWidth(lang.get("mcvmcomputers.pc_editing.add_mbram_btn").replace("%s", "512"))+4;
						ButtonWidget sixfourM = ButtonWidget.builder(Text.literal(lang.get("mcvmcomputers.pc_editing.add_mbram_btn").replace("%s", "64")), (btn) -> this.addRamStick(ItemList.ITEM_RAM64M, 64))
								.dimensions(this.width/2 + 50, this.height / 2 - 64, addMBRamWidth, 12).build();
						ButtonWidget oneM = ButtonWidget.builder(Text.literal(lang.get("mcvmcomputers.pc_editing.add_mbram_btn").replace("%s", "128")), (btn) -> this.addRamStick(ItemList.ITEM_RAM128M, 128))
								.dimensions(this.width/2 + 50, this.height / 2 - 51, addMBRamWidth, 12).build();
						ButtonWidget twoM = ButtonWidget.builder(Text.literal(lang.get("mcvmcomputers.pc_editing.add_mbram_btn").replace("%s", "256")), (btn) -> this.addRamStick(ItemList.ITEM_RAM256M, 256))
								.dimensions(this.width/2 + 50, this.height / 2 - 38, addMBRamWidth, 12).build();
						ButtonWidget fiveM = ButtonWidget.builder(Text.literal(lang.get("mcvmcomputers.pc_editing.add_mbram_btn").replace("%s", "512")), (btn) -> this.addRamStick(ItemList.ITEM_RAM512M, 512))
								.dimensions(this.width/2 + 50, this.height / 2 - 25, addMBRamWidth, 12).build();
						ButtonWidget oneG = ButtonWidget.builder(Text.literal(lang.get("mcvmcomputers.pc_editing.add_ram_btn").replace("%s", "1")), (btn) -> this.addRamStick(ItemList.ITEM_RAM1G, 1024))
								.dimensions(this.width/2 + 50, this.height / 2 - 12, addMBRamWidth, 12).build();
						ButtonWidget twoG = ButtonWidget.builder(Text.literal(lang.get("mcvmcomputers.pc_editing.add_ram_btn").replace("%s", "2")), (btn) -> this.addRamStick(ItemList.ITEM_RAM2G, 2048))
								.dimensions(this.width/2 + 50, this.height / 2 + 1, addMBRamWidth, 12).build();
						ButtonWidget fourG = ButtonWidget.builder(Text.literal(lang.get("mcvmcomputers.pc_editing.add_ram_btn").replace("%s", "4")), (btn) -> this.addRamStick(ItemList.ITEM_RAM4G, 4096))
					...
					if(pc_case.getGigsOfRamInSlot0() > 0) {
						...
						if(pc_case.getGigsOfRamInSlot0() < 1000 && pc_case.getGigsOfRamInSlot0() >= 100) {
							context.drawText(this.textRenderer, (int) pc_case.getGigsOfRamInSlot0() + " MB", this.width/2+4, this.height/2+2, -1, false);
						}else if(pc_case.getGigsOfRamInSlot0() < 100) {
							context.drawText(this.textRenderer, (int) pc_case.getGigsOfRamInSlot0() + " MB", this.width/2+10, this.height/2+2, -1, false);
						}else {
							context.drawText(this.textRenderer, (int) (pc_case.getGigsOfRamInSlot0()/1024) + " GB", this.width / 2 + 16, this.height / 2 + 2, -1, false);
						}
						ms.pop();
					...
					if(pc_case.getGigsOfRamInSlot1() > 0) {
						...
						if(pc_case.getGigsOfRamInSlot1() < 1000) {
							context.drawText(this.textRenderer, (int) pc_case.getGigsOfRamInSlot1() + " MB", this.width/2+42, this.height/2+2, -1, false);
						}else {
							context.drawText(this.textRenderer, (int) (pc_case.getGigsOfRamInSlot1()/1024) + " GB", this.width / 2+42, this.height / 2+2, -1, false);
						}
						ms.pop();
					...
					int thirtyTwoWidth = textRenderer.getWidth(lang.get("mcvmcomputers.pc_editing.add_32bit_mobo"))+4;
					ButtonWidget thirtytwo = ButtonWidget.builder(Text.literal(lang.get("mcvmcomputers.pc_editing.add_32bit_mobo")), (btn) -> this.addMotherboard(false))
					...
					ButtonWidget sixtyfour = ButtonWidget.builder(Text.literal(lang.get("mcvmcomputers.pc_editing.add_64bit_mobo")), (btn) -> this.addMotherboard(true))
				...
				if(turnedOn) {
					...
					this.addDrawableChild(ButtonWidget.builder(Text.literal(lang.get("mcvmcomputers.pc_editing.turn_off")), this::turnOffPC)
					...
					this.addDrawableChild(ButtonWidget.builder(Text.literal(lang.get("mcvmcomputers.pc_editing.turn_on")), this::turnOnPC)
							.dimensions((this.width/2 + 103) - buttonW, this.height / 2 - 80, buttonW, 12).build());
				}
				...
				if(pc_case.getIsoFileName().isEmpty()) {
					ms.push();
					ms.translate(0, 0, 200);
					context.drawText(this.textRenderer, lang.get("mcvmcomputers.pc_editing.select_iso"), this.width/2 - 75, this.height/2 - 75, -1, false);
					ms.pop();
					...
					ms.push();
					ms.translate(0, 0, 200);
					context.drawText(this.textRenderer, lang.get("mcvmcomputers.pc_editing.inserted_iso"), this.width/2 - 75, this.height/2 - 75, -1, false);
					...
					this.addDrawableChild(ButtonWidget.builder(Text.literal(lang.get("mcvmcomputers.pc_editing.eject")), (btn) -> removeISO())
				...
				ButtonWidget bw = ButtonWidget.builder(Text.literal(lang.get("mcvmcomputers.pc_editing.open_case")), (btn) -> openCase = true)
		...
		context.drawText(this.textRenderer, lang.get("mcvmcomputers.pc_editing.close"), 4, 4, -1, false);
		ms.pop();
	}
