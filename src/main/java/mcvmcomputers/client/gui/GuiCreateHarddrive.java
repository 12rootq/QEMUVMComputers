...
public class GuiCreateHarddrive extends Screen {
	...
	public void init() {
		if(currentState == State.CREATE_NEW) {
			...
			this.addSelectableChild(hddSize); // Добавляем только для взаимодействия (рендерится вручную ниже)
			...
			this.addDrawableChild(ButtonWidget.builder(Text.literal(translation("mcvmcomputers.vhd_setup.newvhd")), (btn) -> createNew())
			...
			this.addDrawableChild(ButtonWidget.builder(Text.literal(translation("mcvmcomputers.vhd_setup.menu")), (btn) -> switchState(State.MENU))
					.dimensions(this.width - (menuWidth+10), this.height - 30, menuWidth, 20).build());
		}else if(currentState == State.MENU) {
			int newvhdWidth = textRenderer.getWidth(translation("mcvmcomputers.vhd_setup.newvhd"))+40;
			this.addDrawableChild(ButtonWidget.builder(Text.literal(translation("mcvmcomputers.vhd_setup.newvhd")), (btn) -> switchState(State.CREATE_NEW))
			...
			this.addDrawableChild(ButtonWidget.builder(Text.literal(translation("mcvmcomputers.vhd_setup.oldvhd")), (btn) -> switchState(State.SELECT_OLD))
			...
			for(File f : files) {
				this.addDrawableChild(ButtonWidget.builder(Text.literal((f.getName() + " | " + ((float)f.length()/1024f/1024f) + " " + translation("mcvmcomputers.vhd_setup.mb_used"))), this::selectOld)
	...
	private void selectOld(ButtonWidget wdgt) {
		// Pattern.quote используется для точного совпадения, так как "|" - служебный символ в регулярках
	...
	public void render(DrawContext context, int mouseX, int mouseY, float delta) {
		...
		if(currentState == State.CREATE_NEW) {
			...
			context.drawText(this.textRenderer, translation("mcvmcomputers.vhd_setup.vhdsize"), this.width/2-150, this.height/2-20, -1, false);
