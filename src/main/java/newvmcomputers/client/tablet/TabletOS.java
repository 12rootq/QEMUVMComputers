...
public class TabletOS {
	...
	private final ArrayList<Float> radarRadius; 
	private float totalTimeRadar;
	private BufferedImage lastRadarImage; 
	private boolean satelliteVisible = false;
	public final OrderingTabletModel orderingTabletModel = new OrderingTabletModel(OrderingTabletModel.getTexturedModelData().createModel());
	...
	public TabletOS() throws FontFormatException, IOException {
		...
		font = Font.createFont(Font.PLAIN, mcc.getResourceManager().getResource(new Identifier("newvmcomputers", "font/tabletfont.ttf")).get().getInputStream());
	...
	private void update() {
		...
		if(tabletOn) {
			...
			orderingTabletModel.setButtons(
					pressed(GLFW.GLFW_KEY_UP),
					pressed(GLFW.GLFW_KEY_DOWN),
					pressed(GLFW.GLFW_KEY_LEFT),
					pressed(GLFW.GLFW_KEY_RIGHT),
					pressed(GLFW.GLFW_KEY_ENTER),
					deltaTime
			);
