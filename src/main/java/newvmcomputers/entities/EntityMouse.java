...
public class EntityMouse extends Entity {
	private static final EntityDataAccessor<Float> LOOK_AT_POS_X =
			SynchedEntityData.defineId(EntityMouse.class, EntityDataSerializers.FLOAT);
	private static final EntityDataAccessor<Float> LOOK_AT_POS_Y =
			SynchedEntityData.defineId(EntityMouse.class, EntityDataSerializers.FLOAT);
	private static final EntityDataAccessor<Float> LOOK_AT_POS_Z =
			SynchedEntityData.defineId(EntityMouse.class, EntityDataSerializers.FLOAT);

	public EntityMouse(EntityType<?> type, Level level) {
		super(type, level);
	}

	public EntityMouse(Level level, double x, double y, double z) {
		this(EntityList.MOUSE.get(), level);
		this.setPos(x, y, z);
	}

	public EntityMouse(Level level, Double x, Double y, Double z, Vec3 lookAt, String uuid) {
		this(EntityList.MOUSE.get(), level);
		this.setPos(x, y, z);
