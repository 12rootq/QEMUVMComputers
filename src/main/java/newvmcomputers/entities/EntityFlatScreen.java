...
public class EntityFlatScreen extends Entity {
	private static final EntityDataAccessor<Float> LOOK_AT_POS_X =
			SynchedEntityData.defineId(EntityFlatScreen.class, EntityDataSerializers.FLOAT);
	...
	private static final EntityDataAccessor<String> OWNER_UUID =
			SynchedEntityData.defineId(EntityFlatScreen.class, EntityDataSerializers.STRING);

	public EntityFlatScreen(EntityType<?> type, Level level) {
		super(type, level);
	}

	public EntityFlatScreen(Level level, double x, double y, double z) {
		this(EntityList.FLATSCREEN.get(), level);
		this.setPos(x, y, z);
	}

	public EntityFlatScreen(Level level, Double x, Double y, Double z, Vec3 lookAt, String uuid) {
		this(EntityList.FLATSCREEN.get(), level);
		this.setPos(x, y, z);
